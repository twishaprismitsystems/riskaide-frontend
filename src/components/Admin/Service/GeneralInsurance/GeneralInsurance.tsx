...
export default function GeneralInsurance() {
    ...
    const { fields, append, remove } = useFieldArray({
        ...
        name: "generalInsurance.data",
    });

    return (
        <>
            <AdminHeader title='General Insurance Section' />
            <div className="mt-8">
                <RHFTextField
                    title={"Title"}
                    name={`generalInsurance.title`}
                    isRequired={true}
                    placeHolder="Please enter title"
                />
            </div>
            <AdminSubHeading title="General Insurance Item" />
            {fields.map((item, index) => (
                <div key={item.id} className="pt-8 grid  grid-cols-1 sm:grid-cols-3 gap-8">
                    <RHFTextField
                        title={"Title"}
                        name={`generalInsurance.data.${index}.title`}
                        isRequired={true}
                        placeHolder="Please enter title"
                    />
                    <RHFTextField
                        title="Description"
                        name={`generalInsurance.data.${index}.desc`}
                        isRequired={true}
                        isTextArea={true}
                    ...
                    <div className="flex items-center">
                        <RHFImageUploader showImageRequiredMessage={true}
                            savePath={`generalInsurance.data.${index}.image`}
